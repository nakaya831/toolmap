...
| --- | --- | --- |
| 1.0 | 2026-09 | 初版 |
| 2.1 | 2026-09-13 | 大項目を「① 分野から学ぶ」「② 種別から学ぶ」の2軸に。種別（layer）をコレクション `layers` として持ち、種別ごとの意味・種別同士の関係・選び方を解説するページ `/layers/`、`/layers/[id]/` を新設。種別「実行環境」の代表として Node.js を追加（53件） |
| 2.0 | 2026-09-13 | 発注者の方向転換を反映。前提を「初学者が学習するためのサイト」に置き直す。課題シナリオ（`scenarios`）を廃止。分類に「初学者向けの説明」「隣接分野とのつながり」「最初に触るツール」を持たせ、全体像ページ `/map/` を新設。ツール一覧 `/tools/` を全量表示に |
...
**プログラミングを学び始めた人が、ソフトウェアを構成する分野の全体像と、分野同士のつながりを理解し、各分野の代表的な手段が何をできて何をできないかを知る**ための学習サイトをつくる。

初学者が最初につまずくのは、個々のツールの使い方ではなく、「言語・フロントエンド・バックエンド・データベース・ホスティング……という言葉が、それぞれ何を指し、どうつながって一つのソフトウェアになるのか」が見えないことである。本サイトはまずその地図を与え、そのうえで各手段の制約を同じ物差しで示す。

最終的な成功像は、利用者が以下を自力で言語化できることである。

- ソフトウェアはどんな分野の部品でできていて、それぞれがどうつながっているか
- ある分野で最初に触るべき手段はどれで、次に何を学べばよいか
- 各手段が何をできて何をできないか、破綻する条件（制約）は何か

課題（「毎朝集計を Slack に流したい」）を起点にする入口は 1.x で設けたが、初学者には課題の粒度が限定的すぎて使えないと判断し、2.0 で廃止した。
...
| 各ツールのチュートリアル提供 | 公式ドキュメントに劣後する。本サイトは「何であるか・どうつながるか・何ができないか」までを担い、「どう使うか」は一次情報への導線に徹する |
| 構文リファレンス | 検索とAIで代替済み |
...
- 主：プログラミングを学び始めた初学者。言語を1つ触り始めた段階で、「フロントエンド」「バックエンド」「デプロイ」といった言葉の関係がまだ見えていない人（＝発注者自身）
...
| ID | シナリオ | 起点となる画面 |
| --- | --- | --- |
| S1 | ソフトウェアがどんな部品でできているか、全体像を知りたい | 全体像（`/map/`） |
| S2 | ある分野が何をする分野で、隣の分野とどうつながるかを知りたい | 分類詳細 |
| S3 | その分野で最初に何を触ればよいかを知りたい | 分類詳細（「最初に触るなら」） |
| S4 | 特定ツールの限界を知りたい。「Lambdaの実行時間上限は」 | ツール詳細 |
| S5 | 未知の単語に遭遇した。「Terraformとは何か」 | 検索 / ツール一覧 → ツール詳細 |
| S6 | 「定期実行したい」のような目的から手段を知りたい | 能力一覧 |
...
| `capabilities` | 能力（できること） | 目的とツールを接続する語彙 |
| `categories` | 分類 | 俯瞰の単位（軸①「分野」）。2.0 で初学者向けの説明・隣接分野とのつながり・最初に触るツールを持つ |
| `layers` | 種別 | 軸②「種別」。`tools.layer` の6つの列挙値と同じ ID を持ち、種別の意味・他の種別との関係・選び方を解説する |

`scenarios`（課題シナリオ）は 2.0 で廃止した。

### 4.1.1 二つの軸（2.1）

| 軸 | 分ける基準 | 問い | ページ |
| --- | --- | --- | --- |
| ① 分野（categories） | 部品の**役割**。何のための部品か | 「画面」「サーバー」「データの保存」はどうつながるか | `/categories/`、`/map/` |
| ② 種別（layers） | 道具の**性質**。自分で書く土台か、動かすものか、借りるサービスか | 同じ分野の中で、何と何を比べているのか | `/layers/` |

ツールは必ず1つの分野と1つの種別に属する。比較表は分野の中を種別で分けて表示し（6.3）、種別ページは種別の中を分野で分けて表示する。
...
const categories = defineCollection({
  ...
  schema: z.object({
    name: z.string(),
    order: z.number(),               // 学習の推奨順。全体像ページと一覧はこの順に並ぶ
    question: z.string(),            // このカテゴリが答える問い
    summary: z.string(),
    plain: z.string(),               // 初学者向けの一言。専門用語を使わずに「何をする分野か」
    role: z.string(),                // 全体像の中での役割。図の説明文になる
    connections: z.array(z.object({  // 隣接分野とのつながり。図の矢印になる
      to: reference('categories'),
      how: z.string(),               // 「画面は API を呼んでデータを受け取る」のように動詞で
    })).min(1),
    startWith: reference('tools'),   // この分野で最初に触るツール
    startWhy: z.string(),            // その理由。1〜2文
  }),
});
// 本文（Markdown）に、初学者向けの解説を 3〜6 段落で書く：
// この分野が扱う問題 / よく出る言葉の意味 / 隣の分野との境界 / 最初の一歩
...
初学者には分類名の列挙だけでは「それぞれが何を指し、どうつながるか」が分からない。そこで各分類は、`plain`（一言の説明）、`role`（全体像の中の役割）、`connections`（隣接分類との関係を動詞で）、`startWith`（最初に触るツール）を持ち、全体像ページ `/map/` はこれらから図と説明を生成する。

全体像の骨格は次の通り。図はこの骨格を SVG で描く。

```
                 ┌────────────────────────────────────────────┐
  利用者 ──────▶ │ 画面・フロントエンド ◀──▶ サーバー・API ◀──▶ データの保存 │  ← 言語・実行環境で書く
                 └────────────────────────────────────────────┘
                                   ▲ 載せる
                 ┌────────────────────────────────────────────┐
                 │ 実行基盤・ホスティング                         │  ← 開発基盤・CI/CD で届ける
                 └────────────────────────────────────────────┘      監視・可観測性 で見守る
                                                                    認証・セキュリティ で守る
  周辺：自動化・連携／ノーコード・SaaS（コードを書かずにつなぐ）
  利用：データ処理・分析 → AI・機械学習（溜まったデータを使う）
```

学習の推奨順は `order` で表す：言語 → 画面 → サーバー → データの保存 → 実行基盤 → 開発基盤 → 監視 → 認証 → 自動化 → ノーコード → データ処理 → AI。
...
| パス | 内容 | 生成 |
| --- | --- | --- |
| `/` | トップ。初学者向けの導入、全体像の図、分野の一覧（一言説明つき）、ツール全量の一覧 | 静的 |
| `/map/` | 全体像。分野のつながりを図と文章で解説し、学習の推奨順を示す | 静的 |
| `/categories/` | 全カテゴリ一覧（一言説明・最初に触るツール・掲載ツール） | 静的 |
| `/categories/[id]/` | 分野の解説（何をする分野か／隣とのつながり／最初に触るなら）＋比較表 | 静的 |
| `/layers/` | 種別から学ぶ。6つの種別の意味と関係（図・表）、なぜ種別で分けるか | 静的 |
| `/layers/[id]/` | 種別の解説（一言／役割／いつ選ぶか／他の種別との関係）＋その種別のツールを分野別に一覧 | 静的 |
| `/tools/` | ツール全量の一覧（分野別、さらに種別で小見出し分け、一行定義つき）。1ページに収める必要はない | 静的 |
| `/tools/[id]/` | ツール詳細 | 静的 |
...
| 3 | 完了 | 付録Bの 52 件（第1優先 42 件＋第2優先 10 件）をすべて掲載。全12分類に3件以上。出典つき事実 337 件。シナリオ 6 件は全候補が解決。索引に出ない capability は 7 件（紐づくツール 2 件未満） |
| 5（2.0） | 完了 | 初学者向けへの転換：scenarios 廃止、分類のつながり（`connections` 等）、`/map/`、`/tools/` 全量一覧、トップの再構成 |
| 6（2.1） | 完了 | 大項目を「分野から学ぶ」「種別から学ぶ」の2軸に。`layers` コレクション、`/layers/`、種別の図、Node.js 追加 |
| 4 | 完了 | 検索は 6.5 の3条件に合格。リンク検査 256 URL で到達不能ゼロ（遮断による機械確認不能 2 件）。CI と月次 Issue 起票のワークフローは作成済みだが、リポジトリ未作成のため実行は未確認 |
