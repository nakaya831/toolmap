...
| `categories` | 分類 | 俯瞰の単位。2.0 で初学者向けの説明・隣接分野とのつながり・最初に触るツールを持つ |
...
| `/tools/` | ツール全量の一覧（分野別、一行定義つき）。1ページに収める必要はない | 静的 |
| `/tools/[id]/` | ツール詳細 | 静的 |
...
| 5（2.0） | 実装中 | 初学者向けへの転換：scenarios 廃止、分類のつながり（`connections` 等）、`/map/`、`/tools/` 全量一覧、トップの再構成 |
